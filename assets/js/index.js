...
// SELL OFFERS
fetch(
  "https://tnbcrow-discord-bot.herokuapp.com/advertisement?ordering=price&side=SELL&format=json"
)
  .then((response) =>
...
function displayAdvertisements(offers, wrapper) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    offer.innerHTML = `<td>${
      offers.results[i].price / 100000000
    }</td><td>${numberWithCommas(offers.results[i].amount / 100000000)}</td>`;
    wrapper.appendChild(offer);
  }
}

// Recent Trades
fetch("https://tnbcrow.pythonanywhere.com/recent-trades")
  .then((res) => res.json())
  .then((trades) => {
    if (trades.count > 0) {
...
function displayTrades(trades, wrapper) {
  ...
  for (let i = 0; i < trades.count && i < 8; i++) {
    ...
    trade.innerHTML = `<td>${trades.results[i].rate / 10000}</td><td>${
      trades.results[i].amount
    }</td><td>${parseInt(
      (trades.results[i].rate / 10000) * trades.results[i].amount
    )}</td>`;
...
fetch(
  "https://raw.githubusercontent.com/itsnikhil/tnb-analysis/master/web/js/static.json"
)
  .then((res) => res.json())
  .then((data) => {
...
fetch("https://tnbcrow.pythonanywhere.com/recent-trades")
  ...
  .then((data) => {
    ...
    cap.innerText = numberWithCommas(
      parseInt(circulating_supply * rate.innerText)
    );
  })
  .catch((err) => {
